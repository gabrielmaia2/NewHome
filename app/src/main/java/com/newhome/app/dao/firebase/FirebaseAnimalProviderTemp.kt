...
class FirebaseAnimalProviderTemp(
    ...
    suspend fun getAnimaisList(userId: String, list: String): Deferred<List<AnimalData>> =
        CoroutineScope(Dispatchers.Main).async {
            ...
            val animais = db.runTransaction { transaction ->
                ...
                val idsAnimais = snapshot.data!![list] as List<*>
    ...
    override suspend fun getAnimaisPostosAdocao(donoId: String): Deferred<List<AnimalData>> =
        getAnimaisList(donoId, "animais")

    override suspend fun getAnimaisAdotados(adotadorId: String): Deferred<List<AnimalData>> =
        getAnimaisList(adotadorId, "adotados")

    override suspend fun getAnimaisSolicitados(solicitadorId: String): Deferred<List<AnimalData>> =
        getAnimaisList(solicitadorId, "solicitados")

    override suspend fun getDonoInicial(animalId: String): Deferred<UserData> =
        CoroutineScope(Dispatchers.Main).async {
            ...
            val dono = db.runTransaction { transaction ->
                ...
                if (!donoData.exists()) return@runTransaction UserData.empty
    ...
    override suspend fun getAdotador(animalId: String): Deferred<UserData?> =
        CoroutineScope(Dispatchers.Main).async {
            val docRef = db.collection("animais").document(animalId)
            val usuariosRef = db.collection("usuarios")
            ...
            val adotador = db.runTransaction { transaction ->
                ...
                if (!adotadorData.exists()) return@runTransaction UserData.empty
